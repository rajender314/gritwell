...
export default function Rootcause() {
	...
	function checkPemission(){
		...
		if(checkUserPermissions(userPermissions, 'edit-rootcause') === true){
			setHideEdit(true)
			setHideAdd(true)
			showhypothesisModal(false)
			showAddhypothesisModal(false)
		}
		if(checkUserPermissions(userPermissions, 'import-rootcause') === true){
			setHideDownload(true)
		}
		if(checkUserPermissions(userPermissions, 'export-rootcause') === true){
			setHideUpload(true)
	...
	function onKeyUpValue(e: any) {
		if (e.key === 'Enter') {
			if (e.target.value.length >= 2 || isSearch) {
				...
				rootcause({ clearPageNumber: true });
	...
	async function rootcause(params: any = {}) {
		let page = pageNumber;
		...
		const apiObject: PayloadProps = {
			payload: {},
			method: "GET",
			apiUrl: apiEndpoint.rootcauseApi.concat("?search=" + search + "&page=" + page + "&perPage=" + perPage + "&sort=" + sortDirection + "&column=" + sortk + "&status=" + true),
	...
	async function getRecomendation(id: any) {
		const apiObject: PayloadProps = {
			payload: {},
			method: "GET",
			apiUrl: apiEndpoint.rootcauseApi.concat(id),
			headers: { Authorization: token }
		}
	...
	async function onUpload() {
		...
		return await axios({
			url: apiBaseUrl + apiEndpoint.uploadRootcause,
			method: "POST",
	...
	function onDownload() {
		window.open(apiBaseUrl + apiEndpoint.downloadRootcause + `?search=&token=${noBearerToken}`, "_self")
	}
	...
	async function setSortPros(e: SortProps) {
		...
		console.log(e)
		rootcause({ sortKey: e.accessor, sortDirection: e.sortDirection })
				...
				<RightContainer>
					...
					<GridHolder >
						...
						{(count > 0 || isSearch) && !showSpinner ? <PiRecomendationsHeader
							...
							searchPlaceholder="Search Root Cause"
						...
						/>
							...
							: <> {(count === 0 || !isSearch) && !showSpinner ? <PiNoRecomendations
								...
								onDownloadClick={onDownload}
								onUploadClick={() => openUpload()}
								title="No root cause added yet. Click on download for sample format"
								titleHeader='Root Cause'
								onAddClick={() => showAddhypothesisModal(true)}
								downloadButtonDisplay={hideDownload}
								addButtonDisplay={hideAdd}
								uploadButtonDisplay={hideUpload}
			...
			<SideDrawerHolder>
				...
				>
					...
					{hypothesisModal && <EditHypothesis
						...
						onClick={() => rootcause({ clearSearch: true })}
						userId={userId}
					/>}
