...
export default function Goals() {
	...
	function checkPemission(){
		...
		if(checkUserPermissions(userPermissions, 'edit-goals') === true){
			setHideEdit(true)
			setHideAdd(true)
			showhypothesisModal(false)
			showAddhypothesisModal(false)
		}
		if(checkUserPermissions(userPermissions, 'import-goals') === true){
			setHideDownload(true)
		}
		if(checkUserPermissions(userPermissions, 'export-goals') === true){
			setHideUpload(true)
	...
	async function goals(params: any = {}) {
		let page = pageNumber;
		...
		const apiObject: PayloadProps = {
			payload: {},
			method: "GET",
			apiUrl: apiEndpoint.goalsApi.concat("?search=" + search + "&page=" + page + "&perPage=" + perPage + "&sort=" + sortDirection + "&column=" + sortk + "&status=" + true),
	...
	async function getRecomendation(id: any) {
		const apiObject: PayloadProps = {
			payload: {},
			method: "GET",
			apiUrl: apiEndpoint.goalsApi.concat(id),
			headers: { Authorization: token }
		}
	...
	async function onUpload() {
		...
		return await axios({
			url: apiBaseUrl + apiEndpoint.uploadGoals,
			method: "POST",
	...
	function onDownload() {
		window.open(apiBaseUrl + apiEndpoint.downloadGoals + `?search=&token=${noBearerToken}`, "_self")
	}
	...
	async function setSortPros(e: SortProps) {
		...
		console.log(e)
		goals({ sortKey: e.accessor, sortDirection: e.sortDirection })
					...
					<GridHolder >
						...
						/>
							...
							: <> {(count === 0 || !isSearch) && !showSpinner ? <PiNoRecomendations
								...
								onDownloadClick={onDownload}
								onUploadClick={() => openUpload()}
								title="No goals added yet. Click on download for sample format"
								titleHeader='Goals'
								onAddClick={() => showAddhypothesisModal(true)}
								downloadButtonDisplay={hideDownload}
								addButtonDisplay={hideAdd}
			...
			<SideDrawerHolder>
				...
				>
					...
					{hypothesisModal && <EditHypothesis
						...
						onClick={() => goals({ clearSearch: true })}
						userId={userId}
					/>}
