...
export default function Diagnosis() {
	...
	function checkPemission(){
		...
		if(checkUserPermissions(userPermissions, 'edit-diagnosis') === true){
			setHideEdit(true)
			setHideAdd(true)
			showhypothesisModal(false)
			showAddhypothesisModal(false)
		}
		if(checkUserPermissions(userPermissions, 'import-diagnosis') === true){
			setHideDownload(true)
		}
		if(checkUserPermissions(userPermissions, 'export-diagnosis') === true){
			setHideUpload(true)
	...
	function onKeyUpValue(e: any) {
		if (e.key === 'Enter') {
			if (e.target.value.length >= 2 || isSearch) {
				...
				diagnosis({ clearPageNumber: true });
	...
	async function diagnosis(params: any = {}) {
		let page = pageNumber;
		...
		const apiObject: PayloadProps = {
			payload: {},
			method: "GET",
			apiUrl: apiEndpoint.diagnosisApi.concat("?search=" + search + "&page=" + page + "&perPage=" + perPage + "&sort=" + sortDirection + "&column=" + sortk + "&status=" + true),
	...
	async function getRecomendation(id: any) {
		const apiObject: PayloadProps = {
			payload: {},
			method: "GET",
			apiUrl: apiEndpoint.diagnosisApi.concat(id),
			headers: { Authorization: token }
		}
	...
	async function onUpload() {
		...
		return await axios({
			url: apiBaseUrl + apiEndpoint.uploadDiagnosis,
			method: "POST",
	...
	function onDownload() {
		window.open(apiBaseUrl + apiEndpoint.downloadDiagnosis + `?search=&token=${noBearerToken}`, "_self")
	}
	...
	async function setSortPros(e: SortProps) {
		...
		console.log(e)
		diagnosis({ sortKey: e.accessor, sortDirection: e.sortDirection })
					...
					<GridHolder >
						...
						/>
							...
							: <> {(count === 0 || !isSearch) && !showSpinner ? <PiNoRecomendations
								...
								onDownloadClick={onDownload}
								onUploadClick={() => openUpload()}
								title="No diagnosis added yet. Click on download for sample format"
								titleHeader='Diagnosis'
								onAddClick={() => showAddhypothesisModal(true)}
								downloadButtonDisplay={hideDownload}
								addButtonDisplay={hideAdd}
								uploadButtonDisplay={hideUpload}
			...
			<SideDrawerHolder>
				...
				>
					...
					{hypothesisModal && <EditHypothesis
						...
						onClick={() => diagnosis({ clearSearch: true })}
						userId={userId}
					/>}
