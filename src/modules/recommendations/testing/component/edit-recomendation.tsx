...
export default function EditRecommendation(props: any) {
	...
	function handleSubmit(data: any) {
		...
		const apiObject: PayloadProps = {
			...
			apiUrl: props.userId ? apiEndpoint.testingApi.concat(props.userId) : apiEndpoint.testingApi,
...
const RecommendationComponent = ({ formik, onCancel, userid, initialvalues, onFileupdate, setIsChecked, isChecked, toggleChecked, setToggleChecked, ...props }: any) => {
	...
	const handleChange = (e: any, type: string) => {
		...
		if (type === "name") {
			...
			btnDisable(initialvalues.type, e.target.value)
		} else if (type === "testdetails") {
				...
				<FormInnerContainer>
					<UserDataInfo >
						...
						<div style={{ paddingRight: "32px" }}>
							...
							<FormField className="one-sibling gw-required">
								<PiInputForm name="type"
									label="Sample Type"
									onChange={(e: any) => handleChange(e, "sampletype")}
						...
						<div style={{ paddingRight: "32px" }}>
							<FormField className="one-sibling gw-required">
								<PiInputForm name="link"
									label="Test Details"
									onChange={(e: any) => handleChange(e, "testdetails")}
